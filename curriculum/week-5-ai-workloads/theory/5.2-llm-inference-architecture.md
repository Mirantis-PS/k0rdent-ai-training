...
| **Latency (short)** | Blocked by long requests | Immediate | Significantly lower (workload-dependent) |
...
Check precision and usable device memory before selecting a topology. A 405B model needs approximately 810 GB for FP16/BF16 weights alone, so eight 80 GB devices cannot hold those weights. FP8 reduces weight storage to about 405 GB but still requires space for KV cache, activations and runtime overhead. These are capacity examples, not guaranteed serving configurations.
...
| 405B model, FP8 weights | 8 × 80 GB (NVLink) | TP=8 | `tensor_parallel_size=8` |
| 405B model, FP16/BF16 weights | 16 × 80 GB (2 nodes) | TP=8, PP=2 | `tensor_parallel_size=8, pipeline_parallel_size=2` |
...
> **Note:** vLLM V1 (released 2025) introduces an async execution architecture that significantly improves throughput. The concepts in this section apply to both V0 and V1.
