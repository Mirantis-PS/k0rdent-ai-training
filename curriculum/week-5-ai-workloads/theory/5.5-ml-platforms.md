...
Kubeflow provides specialized operators for different training frameworks. The **Kubeflow Trainer v2** (November 2024) introduced a unified `TrainJob` API that supersedes per-framework CRDs:
...
> **Note:** MLflow Projects was removed in MLflow 3.0. AI Gateway (formerly MLflow Deployments for LLMs) is now integrated directly into the MLflow tracking server, providing a unified proxy for LLM API endpoints.
...
> **Note:** MLflow 3.0 removed the stage-based model lifecycle (`Staging`/`Production`/`Archived`/`None`). The replacement is **model aliases** — flexible string labels like `@champion`, `@challenger`, `@candidate` that can be reassigned between versions. Multiple aliases can point to the same version, enabling canary deployments and A/B testing patterns.
...
| **Pipeline Orchestration** | Kubeflow Pipelines | None (removed in 3.0) |
...
|-----------|----------------|---------|
| **MLflow** | `mlflow-1-7-1` | Experiment tracking and model registry |
| **JupyterHub** | `jupyterhub-4-2-0` | Multi-user notebook environment |
| **KServe** | `kserve-v0-15-0` + `kserve-crd-v0-15-0` | Serverless model serving |
| **KubeRay** | `kuberay-operator-1-3-2` | Ray cluster operator for distributed compute |
| **Ray Cluster** | `ray-cluster-1-3-2` | Pre-configured Ray cluster |
...
| **Milvus** | `milvus-5-0-1` | Vector database for embeddings |
...
> **Architecture note:** ServiceTemplates are Kubernetes CRDs sourced from `ghcr.io/k0rdent/catalog/charts`. The `priority` field controls deployment ordering — higher values deploy first — enabling dependencies like GPU Operator before training frameworks. The `clusterSelector` ensures services deploy only to clusters with matching labels.
