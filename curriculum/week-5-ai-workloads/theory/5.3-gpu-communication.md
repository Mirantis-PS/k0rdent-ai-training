...
|---------|---------------|------------|----------|
| Slow inter-node all-reduce | TCP socket transport | `NCCL_DEBUG=INFO` shows "NET/Socket" | Compare with the expected NIC/provider path; check RDMA only on qualified hardware |
...
1. **Use available interconnects**: Prefer NVLink where supported; compare measured bandwidth on the actual hardware using consistent directionality
2. **GPUDirect RDMA**: Enable for multi-node to avoid CPU copies
3. **NCCL Auto-Tunes**: But verify with NCCL_DEBUG=INFO
4. **Match the hardware**: PCIe is expected on A10G; NET/Socket describes inter-node TCP transport, not the intra-node GPU interconnect
...
| **network-operator** | 25.10.0 | OFED drivers, RDMA shared device plugin, SR-IOV, NV-IPAM |
