...
**Domain:** BMaaS / Security / Firmware
...
| Track | Tier | Duration |
|-------|------|----------|
| BMaaS / Security | Required | 2 hours |
...
- [Part 5: Firmware attestation across the boot chain](#part-5-firmware-attestation-across-the-boot-chain)
...
The chain of trust from immutable boot ROM up through userspace is a stack of cryptographic guarantees that engineers commonly hand-wave. This lab is intentionally theory-heavy because Secure Boot + Measured Boot + TPM 2.0 are foundational concepts — you can't validate or troubleshoot them by just toggling on a checkbox.

After the theory, you'll verify Secure Boot enforcement via Redfish, read TPM PCR values with tpm2-tools, capture an attestation quote, and assemble a signed firmware inventory across the boot chain.
...
|--------|----------|----------|----------------|
| TPM 2.0 as a hard requirement | Mandatory on every node — refuse to enroll without it | Strong preference — allow legacy nodes with compensating controls (signed PXE chain + offline attestation) | **Mandatory for new fleet; strong preference with documented exception process for legacy.** See Mirantis open-question discussion below — reality is uneven. Pick mandatory and write down the exception path. |
| Secure Boot key enrollment | Replace OEM PK/KEK with platform-controlled keys (full custody) | Append a platform-owned KEK alongside OEM KEK, keep OEM PK | **Append a platform-owned KEK, keep OEM PK** — full PK replacement breaks vendor firmware-update tooling on many platforms (fwupd capsules, OEM utilities). Appending a platform KEK lets you control db/dbx for tenant boot artifacts while still receiving signed firmware updates. |
...
Mirantis raised this as an open architectural question: *"Do all nodes need TPM 2.0 as a hard requirement?"* It's worth thinking through carefully.

**Pros of mandatory:** a node without a TPM cannot produce a quote, so it cannot satisfy "attested during boot" — there is no substitute. An attacker who replaces firmware on a non-TPM node will never be detected by attestation. Modern AI/GPU hardware (DGX, OEM HGX) all ships with TPM 2.0 — the mandate has zero cost on greenfield.
...
**Recommendation in this curriculum:** mandatory for any node entering a production fleet, with an explicit per-node, time-bounded, compensating-control exception process for documented brownfield cases. Don't let exceptions become permanent by accident.
...
You need both. Secure Boot without Measured Boot means a compromised db boots silently. Measured Boot without Secure Boot means you record the compromise faithfully but never stopped it. The combination is the posture you want.
...
| **KEK** (Key Exchange Keys) | One or more | Platform owner + delegated authorities (Microsoft, vendor, platform operator) | Authorizes changes to db and dbx |
...
Three choices for an operator-managed fleet:

1. **Leave OEM keys in place** — easiest, trusts the Microsoft chain and OEM driver signing. Acceptable for shared infrastructure; weaker isolation than multi-tenant AI infrastructure typically wants.
2. **Append a platform-owned KEK** — keep OEM PK, add your KEK alongside Microsoft's. You can now manage your own db/dbx entries for tenant boot artifacts without breaking OEM firmware updates. **Recommended posture.**
...
`DeployedMode` is the production posture — `SetupMode` accepts a new PK without authentication (fine for first enrollment, never left enabled). Enroll an operator-owned KEK (exact action URI varies by vendor):

```bash
curl -k -u root:... -X POST \
  https://10.99.0.10/redfish/v1/Systems/System.Embedded.1/SecureBootDatabases/KEK/Certificates \
  -H 'Content-Type: application/json' -d @operator-kek.json
```

`operator-kek.json` carries the PEM/DER of the operator-owned KEK certificate. Confirm with a GET on the same collection.
...
## Part 5: Firmware attestation across the boot chain

Every firmware component that runs before the OS is part of the trusted compute base. "Firmware" on a GPU node means more than the host BIOS — every signed component needs an inventory entry, a known-good version, and a verification step:
...
This produces a structured inventory: every component, its version, whether it's signed, whether it's updateable.

### Bringing firmware to a known-good state between tenants

Between tenants, firmware must be reset to known-good — otherwise a previous tenant could leave a modified BMC, NIC, or GPU VBIOS behind. The workflow:

1. Tenant releases the node (Lab 6.4 covers crypto-erase for data; this lab covers firmware).
...
- [ ] operator-owned KEK is present in `/SecureBootDatabases/KEK/Certificates` (if append-mode chosen) or operator-owned PK is enrolled (if full-custody chosen)
- [ ] `tpm2_getcap properties-fixed` shows `TPM2_PT_FAMILY_INDICATOR` is `2.0`
- [ ] Reference PCR set captured for known-good boot (SHA-256 bank, PCRs 0–10) and stored as the attestation baseline
- [ ] `tpm2_checkquote` returns exit 0 against a fresh quote with a fresh nonce
- [ ] Firmware inventory enumerates BIOS, BMC, every NIC, every GPU VBIOS, every NVSwitch tray (where present)
...
| Vendor does not sign their firmware (most often: older NIC or BMC builds) | Vendor lags on the firmware-signing requirement | File a vendor escalation, document the exception with compensating controls, set a deadline; do not silently accept |
...
- **Firmware attestation is a fleet-wide inventory problem, not just a BIOS-signature problem.** BMC, NIC, GPU VBIOS, NVSwitch tray all count. Build the inventory once, run it on every node lifecycle transition.
